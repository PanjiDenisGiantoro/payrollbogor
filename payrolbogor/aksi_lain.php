...
if($_GET['act'] =='insert'){
	...
	if($keterangan==''||$tgl==''||$jumlah==''){
		...
		$simpan = mysqli_query($konek,"insert into lainlains(namaKaryawan,jumlah,keterangan,tgl) values ('$nama','$jumlah','$keterangan','$tgl')");
		if($simpan){
	...
	if($keterangan==''||$tgl==''||$jumlah==''){
		...
		$update = mysqli_query($konek,"update lainlains set status='tidak disetujui',alasan='$alasan' where id='$id'");
$email = mysqli_fetch_array(mysqli_query($konek,"SELECT a.namaKaryawan,b.email,a.id FROM lainlains a JOIN admin b ON a.namaKaryawan = b.namalengkap WHERE a.id = '$_GET[id]'"));
	$email1= $email['email'];
...
$depar = mysqli_fetch_array(mysqli_query($konek,"SELECT b.departemen,a.jumlah,a.tgl,a.keterangan,a.status,a.alasan FROM lainlains a JOIN karyawans b ON a.namaKaryawan = b.nama WHERE a.id = '$_GET[id]'"));
...
<h2 align="center"><font face="Bradley Hand ITC">Pengajuan pinjaman lainlain karyawan<br />
      ...
      <div class="col-12 table-responsive">
       <center> <table class=" table-bordered" border="1">
          <thead>
            <tr>
            <th colspan=7>
              <center><b>Pengajuan  pinjaman lainlain</b></center></th>
            </tr>
          <tr>
            <th colspan="1">Nama</th>
            <th colspan="1">Jabatan</th>
            <th colspan="1">jumlah  pinjaman lain lain</th>
...
$mail->AltBody = 'mohon maaf Anda tidak disetujui dalam Pengajuan  pinjaman lainlain ';
...
$depar = mysqli_fetch_array(mysqli_query($konek,"SELECT b.departemen,a.jumlah,a.tgl,a.keterangan,a.status FROM lainlains a JOIN karyawans b ON a.namaKaryawan = b.nama WHERE a.id = '$_GET[id]'"));
$departemen = $depar['departemen'];
$jumlahPinjam = $depar['jumlahKasbon'];
$tgl = $depar['tgl'];
$waktu = $depar['keterangan'];
...
$mail->MsgHTML($body);
//Replace the plain text body with one created manually
$mail->AltBody = 'Selamat Anda disetujui dalam Pengajuan  pinjaman lainlain ';
